...
VirtualBox and Vagrant sometimes don't play nicely together, depending on the versions installed. Known working configurations which have been tested are:
* VirtualBox: 5.0.28, Vagrant: 1.8.6
* VirtualBox: 5.1.10, Vagrant: 1.8.7
* VirtualBox: 5.1.12, Vagrant: 1.9.1
...
Vagrant will now pull down the relevant Linux dist (currently a Hashicorp-provided 64bit server version of Ubuntu 16.04 - Xenial) image to base the environment on, run it in VirtualBox and configure it. You will be asked which network interface to connect to. It is essential you use the interface that all 'normal' incoming/outgoing traffic on your host goes through (ie. connects to the Internet).
...
Once you've cloned and started the DevEnv, you'll initially be able to see the various Docker containers that make up the system.

You should now be able to type `dc ps`, and see a list of all the services. `dc` is a shell alias for `docker-compose`; the legacy `fig` command still works, as well.
...
Be warned that on all subsequent `vagrant up` commands, you will have to `vagrant ssh` in and then run `dc start` to start all of the services again, as these are stopped on a `vagrant halt`. You may also have to start the previous network services (`start-network`, `haproxy`) if you had to before.
...
By default, the DevEnv allows only for the creation of an application for each device type, but will not allow download of OS images, etc. as the images for these device types are not present within the DevEnv. This is due to space restrictions within the VirtualBox image.
...
As long as you name the host `git.resindev.io`, you should now be able to use the git remote line given at the top of an Application's Dashboard page to be able to add the repository. Pushing is the same as always:
...
# Using `dc` (`docker-compose`) to Develop with Source Repos

`docker-compose` is a Docker container manager that simplifies the creation of applications based around several services. It manages the Docker containers providing the Resin.io backend services, including running and rebuilding them, and does so from a single file called `docker-compose.yml`.
...
1. Uncomment the relevant line from the `docker-compose.yml` manifest (in the `volumes` section for that service)
2. Clone the repo for the service you want to alter code for in the `src` directory, and rename the repo to the service specified in `docker-compose.yml`.
...
4. Restart the relevant service from fig by doing `dc up -d <service>`
...
Some containers pick up changes automatically (for example if they're running `nodemon`). Some need to be manually kicked again using `dc restart <component>`.
