...
We use a combination of Amazon Cloud Formation and CoreOS for running our infrastructure.

1. Amazon Cloud Formation is used to describe the placement, databases capacity and security policies of the machines that that join our CoreOS cluster.
  * This is done via the VPC.template JSON file in the resin-containers/cloudformation directory.
  * The VPC.template controls everything from deciding what machines go behind the load balancer to the policies to be followed for scaling and backup procedures on machine deletion.
...
## Initial Setup
    ...
    Wait for the stack to be created - This usually takes about 30 mins the first run - because of the slow RDS provisioning.
    ...
    Launch all the services by going into the services directory and executing - fleetctl start manager.service Note: we need to change from master to production tag in the services for production deployment by running deployment.sh
...
## Updating containers
    ...
    For setting up auto-update ( Currently disabled )
        ...
        Launch the services in the monitor subfolder - Note: we need to change from master to production tag in the services for production deployment.

        Switch all the HEALTH check ports on ELB to 1234 - Once this is done the infrastructure is in auto - update mode - changing the ELB health check to original values and destroying the monitors brings the infrastructure back to manual.

        The above can be done selectively for services.
    Manual Update Procedure ( Currently used )

        In the "Auto Scaling Groups" of AWS choose the group for the service you are deploying.

        Double the desired number of machines and save.

        Change to the "Load Balancers" section and select the service you are deploying.

        Wait until all the "Status" shows as "X of X instances in service" , so that all the new ones are running.
...
## Gaining access to the instances
    ...
    You can ssh into the manager instance
...
During an upgrade to CoreOS, the CoreOS version is changed in the VPC.template, which immediately re-creates the the manager, git, and vpn instances. This will then drop them out of the etctd cluster because the manager is tho sole leader, and you can't join a cluster without a leader. To do this deploy do the following:
    ...
    Go into EC2 in another tab and detach the EBS volume from the git instance. This won't happen yet.
...
In the case of vpn:
    ...
    Schedule downtime on statuspage.io.
    Do the usual fast-forwarding of production to the selected deploy commit, and push.
    Update CHANGELOG on master. Push.
    Check https://admin.resin.io/top-level-numbers for number of connected devices. Remember this number.
    Run `resinctl build vpn production`
    Wait for the scheduled downtime window.
    Run `sudo resinctl deploy vpn production`
    Once it's up, confirm that the number of connected devices at https://admin.resin.io/top-level-numbers matches pre-deploy numbers.
...
TODO/NOTES
    ...
    The AMI's used are in the beta channel as fleet global loading is currently only supported in beta - Need to revert this once the stable CoreOS[410] changes to release 444.4.0 or above.
